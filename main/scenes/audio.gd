...
func play_from_graph_node(graph_node: GraphNodePlayAudio) -> void:
	_audio_stream_player2d_queue.append(graph_node)
	_play_queue()

func _play_queue() -> void:
	if audio_stream_player2d.playing or _audio_stream_player2d_queue.size() == 0:
		return
	var graph_node: GraphNodePlayAudio = _audio_stream_player2d_queue.front() as GraphNodePlayAudio
	var audio_stream: AudioStream = ResourceLoader.load(graph_node.path)
